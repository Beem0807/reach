...
| Namespace | `secrets` (only the token Secret, by name) | get, update, patch | read/write the agent token. The chart pre-creates the Secret, so no `create`, and access is name-scoped to `tokenSecretName`. |
...
## Token Secret & clean uninstall

The agent claims once and stores its agent token in a Kubernetes Secret
(`tokenSecretName`, default `<release>-token`) so it is shared across replicas and
survives restarts and the 30-day rotation. Mounted Secrets are read-only, so the
agent writes it through the API.

The chart creates this Secret so it is owned by the release and removed on
`helm uninstall` - keeping uninstall/reinstall clean. The agent only patches the
token into it at runtime (an empty Secret reads as "no token yet", so a fresh
install claims normally).
...
| tokenSecretName | string | `""` | Name of the Secret that stores the claimed agent token (shared across replicas, survives restarts/rotation). The chart creates it so Helm owns it and removes it on uninstall; the agent writes the token into it at runtime. Defaults to `<release>-token`. |
