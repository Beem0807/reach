...
## Installing the backend (self-hosted)

The backend setup scripts verify their downloads automatically, the same way `install.sh` does:

- **`local-setup.sh`** (Docker) `cosign verify`s the `nabeemdev/reach` image against this repo's
  `backend.yml` signer identity **before running it**, and checksum-verifies the CLI wheel before
  installing it. `cosign` is optional but recommended - set `REACH_REQUIRE_COSIGN=1` to make a
  missing `cosign` a hard failure (a *failed* verify always aborts).
- **`lambda-setup.sh`** (AWS Lambda) verifies the CloudFormation **template** it is about to deploy
  and the **UI bundle** against the signed `SHA256SUMS` at
  `https://releases.reach.nabeem.com/lambda/<tag>/` before use - a mismatch aborts the deploy - and
  verifies the CLI wheel too. Verifying the packaged template also pins the Lambda code (referenced
  by content-hashed S3 key).

Both mandate the checksum and add the cosign signature check when `cosign` is installed. On a fork,
point verification at your repo with `REACH_REPO=owner/repo`.

**Verify the setup script itself before `curl | bash`** (recommended for production). The scripts are
signed by the `scripts.yml` workflow and published with a `SHA256SUMS` at the bucket root:

```bash
B=https://releases.reach.nabeem.com
curl -fsSLO "$B/lambda-setup.sh"           # or local-setup.sh
curl -fsSLO "$B/SHA256SUMS" -O "$B/SHA256SUMS.sig" -O "$B/SHA256SUMS.pem"
cosign verify-blob --certificate SHA256SUMS.pem --signature SHA256SUMS.sig \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com \
  --certificate-identity-regexp "^https://github.com/Beem0807/reach/\.github/workflows/scripts\.yml@" \
  SHA256SUMS
sha256sum --ignore-missing -c SHA256SUMS    # authenticates the script
bash lambda-setup.sh
```

Note the signer here is **`scripts.yml`** (the setup scripts are unversioned, published on their own),
whereas the artifacts a script *downloads* (template, UI bundle, wheel, agent binaries) are signed by
`backend.yml` / `cli.yml` / `agent.yml`.

## What every release publishes

- **Formal GitHub Release per tag**, with auto-generated notes. A backend/agent **code** merge cuts
  **two** - `<comp>-<ver>` (code artifacts) and `<comp>-helm-<chartVer>` (the chart) - each
  self-contained with its own checksums, signature, and provenance. A chart-only merge cuts just the
  `*-helm-*` release.
- **`SHA256SUMS`** over that release's file artifacts, plus a **keyless cosign signature**
  (`SHA256SUMS.sig`) and its short-lived certificate (`SHA256SUMS.pem`). Verifying the checksums
  file authenticates every file in the release. The same `SHA256SUMS` (+ `.sig`/`.pem`) is mirrored
  to the CDN next to the artifacts the setup scripts fetch - `agent/v<ver>/`, `lambda/v<ver>/`
  (backend template + UI bundle), `cli/v<ver>/` - so deploys can verify without the GitHub Release.
