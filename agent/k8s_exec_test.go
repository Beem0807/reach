...
func TestExecuteK8sBlocksLocalFileReads(t *testing.T) {
	...
	// Direct file read via a filter -> blocked.
	...
	// check (they may still fail to exec kubectl in CI, but must not be Blocked).
	for _, c := range []string{
		"kubectl get pods | grep my-pod",
		"kubectl get pods | grep app/web",
		"kubectl apply -f -",
	} {
		if r := executeK8sCommand(c, allowed); r.Blocked {
			t.Fatalf("non-file arg should not be blocked: %q -> %s", c, r.Stderr)
		}
	}
}
