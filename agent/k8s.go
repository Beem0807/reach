...
// maxPermBytes caps the serialized permission snapshot. DynamoDB items are capped
// at 400KB; we stay well under so the backend write can never fail, even for an
// (over-privileged) agent with broad distinct grants across many namespaces.
const maxPermBytes = 256 * 1024
...
// rules) and marks the snapshot incomplete - so the operator sees a "partial"
// signal rather than a silently failed report. Deterministic: same input, same
// truncation, same hash.
