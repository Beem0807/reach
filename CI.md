...
4. **Release integrity** ([composite action](.github/actions/release-integrity)) - run **once per
...
5. **Publish the signed `SHA256SUMS` to the CDN** next to the artifacts the setup scripts fetch -
...
6. **Tag** the repo (annotated).
7. **Invalidate the CDN** for the mutable paths (no-op until `RELEASES_CF_DISTRIBUTION_ID` is set).
...
- **SBOMs** - SPDX per image (attached as a cosign attestation) + a source SBOM on the code Release.
